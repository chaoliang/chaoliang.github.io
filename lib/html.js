...
export function page({ config, path, title, description, content, faq = null, buildDate }) {
  const canonical = config.siteUrl.replace(/\/$/, '') + path;
  const faqLd = faq && faq.length ? `<script type="application/ld+json">${JSON.stringify({
